...
class IphoneDiscountProcessor implements CartProcessorInterface
{
    ...
    public function process(CartDataCollection $data, Cart $original, Cart $toCalculate, SalesChannelContext $context, CartBehavior $behavior): void
    {
        ...
        $discountLineItem = $this->createDiscount('IPHONE_DISCOUNT');
    ...
    private function findExampleProducts(Cart $cart): LineItemCollection
    {
        return $cart->getLineItems()->filter(function (LineItem $item) {
            ...
            $exampleInLabel = stripos($item->getLabel(), 'iphone') !== false;
    ...
    {
        $discountLineItem = new LineItem($name, 'iphone_discount', null, 1);

        $discountLineItem->setLabel(' Apple Products discount!');
